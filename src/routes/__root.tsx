...
function RootComponent() {
	...
	return (
		<RootDocument>
			{!isAdminRoute && !isAuthRoute && !isPublicRoute && (
				<header className="border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 sticky top-0 z-50 w-full">
					<div className="container flex h-14 items-center justify-between px-6">
						<Link to="/" className="flex items-center gap-2.5 font-bold text-lg text-foreground hover:text-foreground/80 transition-colors">
							<img src="/logo.webp" alt="Logo" className="h-7 w-auto object-contain" />
							<span className="text-base tracking-tight font-extrabold">Bebas Pustaka FKG Unhas</span>
						</Link>
						{/* Desktop Navigation */}
			...
			<main className={(isAdminRoute || isPublicRoute || isAuthRoute) ? "" : "container py-6 px-6"}>
				<Outlet />
			</main>
